...
public class Path {
	...
	class Line {
		...
		Line(double[][] c) {
			...
			is = i.shift(angle, width);
			fs = f.shift(angle, width);
		}
	...
	int shade() {
		...
		double dp = Math.abs(dotProduct(vx, vy, vz, 1, 1, 1)) / Math.sqrt(3);
		return (int) (250 * dp);
	...
	static boolean exists(double[] p) {
		return (p != null && (p[0] > 0 || p[1] > 0));
	}

	double perpindicularDistance(double x, double y) {
		return line.i.distanceFrom(x, y)
				* Math.sin(line.i.angleWith(x, y) - line.angle);
	}

	// prevent driving out of bounds
	public double[] onto(double x, double y) {
		double d = perpindicularDistance(x, y);
		if (d > width) {
			d = d - width;
